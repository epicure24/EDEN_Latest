...
def getId(lat, lng, level):
    return str(s2.CellId.from_lat_lng(getLatLng(lat, lng)).parent(level).id())
...
def computeHeatMap(trace,level,name):
    ...
    count = pd.concat([count, 
                       pd.DataFrame([total]),
                       pd.DataFrame([timeOfHeatMap]),
                       pd.DataFrame([hourOfHeatMap]),
                       pd.DataFrame([centerLatOfHeatMap]),
                       pd.DataFrame([centerLngOfHeatMap])
                      ], ignore_index=True)
